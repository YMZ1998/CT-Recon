...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    // calculate delta Pi
    //////////////////////
    double dPi = static_cast<double>(
        2 * af::Pi /
        (params
             .numImg));  // if 0?to 360?including --> params.numImg-1 else params.numImg
    printf(
    ...
    af::array DarkImg(
        params.win_height,
        params.win_width);  // only 2D image may be averaged over more
    DarkImg = 0.0f;
    ...
    for (int i = params.startIdx; i < params.numUse + params.startIdx; i++) {
      ...
      sprintf_s(filename_buffer, "%s/%s%d.tif", params.indir.c_str(),
                params.fImg.c_str(), i);
      printf_s("%02d)    %s\n", i, filename_buffer);
      // load image
      ...
      FlatImg(af::span, af::span, i - params.startIdx) =
          FIm.cols(params.col, params.col + params.win_width - 1)
              .rows(params.row, params.row + params.win_height - 1)
              .as(f64);
    }
    ...
    for (int i = params.startIdx; i < params.numUse + params.startIdx; i++) {
      ...
      // array indixing needs 0-indexing
      // apply cropping of image
      RawImg(af::span, af::span, i - params.startIdx) =
          RIm.cols(params.col, params.col + params.win_width - 1)
              .rows(params.row, params.row + params.win_height - 1)
              .as(f64);
    }
    ...
    for (int i = params.startIdx; i < params.numDark + params.startIdx; i++) {
      ...
      // array indixing needs 0-indexing
      // apply cropping of image
      DarkImg += DIm.cols(params.col, params.col + params.win_width - 1)
                     .rows(params.row, params.row + params.win_height - 1)
                     .as(f64);
    }
    ...
    af::array PhiRaw = af::atan2(RawArgsMatrix(af::span, af::span, 1),
                                 RawArgsMatrix(af::span, af::span, 2));
    printf(" done.\n");
    ...
    af::array PhiFlat = af::atan2(FlatArgsMatrix(af::span, af::span, 1),
                                  FlatArgsMatrix(af::span, af::span, 2));
    ...
    //////////////////////////////
    af::array AbsorptionImage = RawArgsMatrix(af::span, af::span, 0) /
                                FlatArgsMatrix(af::span, af::span, 0);

    // calculate scatter image
    ///////////////////////////
    printf("\n\tCalculating scatter and absorption image ...");
    af::array ScatAmpRaw = af::sqrt(RawArgsMatrix(af::span, af::span, 1) *
                                        RawArgsMatrix(af::span, af::span, 1) +
                                    RawArgsMatrix(af::span, af::span, 2) *
                                        RawArgsMatrix(af::span, af::span, 2));
    af::array ScatAmpFlat = af::sqrt(FlatArgsMatrix(af::span, af::span, 1) *
                                         FlatArgsMatrix(af::span, af::span, 1) +
                                     FlatArgsMatrix(af::span, af::span, 2) *
                                         FlatArgsMatrix(af::span, af::span, 2));
    ...
    sprintf_s(outfilename, "%s/%02d_of_%02d_PhaseImage.tif",
              params.outdir.c_str(), params.numUse, params.numImg);
    printf("    %s\n", outfilename);
    af::saveImageNative(outfilename, PhiDiff);
    sprintf_s(outfilename, "%s/%02d_of_%02d_ScatterImage.tif",
              params.outdir.c_str(), params.numUse, params.numImg);
    printf("    %s\n", outfilename);
    af::saveImageNative(outfilename, ScatImage);
    sprintf_s(outfilename, "%s/%02d_of_%02d_AbsorptionImage.tif",
              params.outdir.c_str(), params.numUse, params.numImg);
    printf("    %s\n", outfilename);
