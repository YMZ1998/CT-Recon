...
int main(int argc, char* argv[]) {
  ...
  if (gpuPropV.size() < 1) {
    cout << "No Cuda enabled device --> found: " << gpuPropV.size()
         << " device(s)\n";
    exit(-3);
  } else {
    if (gpuPropV.size() > 1) {
  ...
  unsigned int totalMemGpu =
      KBYTE(KBYTE(imageWidth * imageHeight * sizeof(float)) * numOfProjections);
  ...
  if (binFactor == 1) {
    reconVolMemMB =
        KBYTE(KBYTE(sizeof(float) * volXLength * volYLength) * volZLength);
  } else {
    reconVolMemMB = KBYTE(
        KBYTE(sizeof(float) * volXLength / binFactor * volYLength / binFactor) *
        volZLength / binFactor);
  }
  ...
  //////////////////////////////////////////
  QSettings SoftwareSettings(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\EMPA_SOFTWARE\\Recon_Software",
  ...
  if (binFactor == 1) {
    ...
    if (winOrigX + volXLength - imageWidth > FLT_EPSILON) {
      ...
      cout << "   Reset volume reconstruction length in x direction to: "
           << imageWidth - winOrigX << endl;
      backProjParam.volX = volXLength = imageWidth - static_cast<int>(winOrigX);
      if (imageWidth - winOrigX < 1) {
        cout << "   Offset to sub volume origin point exceeds volume dimension "
                "--> Exit\n";
        exit(-1);
      }
    }
    if (winOrigY + volYLength - imageHeight > FLT_EPSILON) {
      cout << "   Warning: Selected window height >> " << winOrigY + volYLength
           << " << exceeds maximal size!\n";
      cout << "   Reset volume reconstruction length in y direction to: "
           << imageHeight - winOrigY << endl;
      backProjParam.volY = volYLength =
          imageHeight - static_cast<int>(winOrigY);
    ...
    if (winOrigZ + volZLength - imageWidth > FLT_EPSILON) {
      ...
      cout << "   Reset volume reconstruction length in z direction to: "
           << imageWidth - winOrigZ << endl;
  ...
  if (para.tomoBlockWrite) {
    ...
    QFile file(val);
    if (!file.exists()) {
      cerr << ">>> Error! << Tomogram Block File: " << val.toStdString()
           << " does not exist.\n";
      return -1;
    }
    qint64 sz = file.size();
  ...
  if (para.sino32BlockUse) {
    ...
    QFile file(val);
    if (!file.exists()) {
      cerr << ">>> Error! << Pre-Processed Sinogram Block File: "
           << val.toStdString() << " does not exist.\n";
      return -1;
    }
    qint64 sz = file.size();
    // data from parameter file
  ...
  FILE* fpVol = 0;
  if (para.tomoBlockWrite) {
    cout << "  >> using tomo block file: " << para.tomoBlockFile.c_str()
         << endl;
  ...
  if (backProjParam.outDataType == "byte") {
    ...
    if (!backProjParam.tomoBlockWrite && !backProjParam.convertBlockInternal) {
      ...
      fflush(stdout);
      fflush(stderr);
    } else if (backProjParam.tomoBlockWrite &&
               !backProjParam.convertBlockInternal) {
      ...
      ConvertBlockInternalTomoDataUC8(backProjParam, maxGrayGlobal,
                                      minGrayGlobal);
      printf("  Converting finished\n");
      fflush(stdout);
      fflush(stderr);
  ...
  if (backProjParam.outDataType == "short16") {
    ...
    if (!backProjParam.tomoBlockWrite && !backProjParam.convertBlockInternal) {
      ...
      fflush(stderr);
               ...
               backProjParam.convertBlockInternal) {
      // float block internal convert block
      ConvertBlockInternalTomoDataI16(backProjParam, maxGrayGlobal,
                                      minGrayGlobal);
      printf("  Converting finished\n");
      fflush(stdout);
      fflush(stderr);
...
#define USE_PARALLEL
      ...
      fflush(stderr);
               ...
               backProjParam.convertBlockInternal) {
      // float block internal convert block
      ConvertBlockInternalTomoDataUI16(backProjParam, maxGrayGlobal,
                                       minGrayGlobal);
      printf("  Converting finished\n");
      fflush(stdout);
      fflush(stderr);
