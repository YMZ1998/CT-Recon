 ...
 public:
  ...
  short operator()(float inVal) {
    return static_cast<short>(
        ROUND((inVal - m_f32min) * m_mapFactor + SHRT_MIN));
  }
...
void InitAngleTable(const BackProjParam& param) {
  if (param.angleList.size() > MAX_PROJ_NUM) {
    cout << "Number of angle table entries (" << param.angleList.size()
         << ") exceeds pre-defined max.size of " << MAX_PROJ_NUM << endl;
    exit(-2);
  }
  ...
  checkCudaErrors(cudaMemcpyToSymbol(radsConst, &rads_temp[0],
                                     sizeof(float) * rads_temp.size()));
}

int runFDK(BackProjParam param, int devNo, float* minGray, float* maxGray,
           FILE* fpVol) {
  //cudaSetDevice(0);
  ...
  tmp_c->voxSize = param.binFac * param.pixSize /
                   param.magnification;  // pixel-size / magnification
  ...
  cout << "Volume dimensions: [" << param.volX << "," << param.volY << ","
       << param.volZ << "]\n";
  cout.flush();
  ...
  for (int nck = 0; nck < number_y_chunks; nck++) {
    ...
    unsigned long long sinChunkSize =
        (size_t)param.imgWidth * (size_t)param.imgHeight * (size_t)nProjUse;
    float* h_sinoData = new float[sinChunkSize]();
    //-ju- allocate memory for backprojection for one slice on host

    /*-ju-27-Oct-2011 new */
    unsigned long long chunkSize = (unsigned long long)param.volX *
                                   (unsigned long long)param.volZ *
                                   (unsigned long long)backproj_chunks;
#ifdef _ASYNC_IO
    ...
    for (int np = 0; np < proj_blcks; np++)  // projection blocks
    {
      ...
      int rval;
      if (param.useFileList == 1) {
        if ((rval = ReadChunkOfSinogramsFromFileList(
                 start_value, end_value, param, &h_sinoData[0])) != 0) {
          fprintf(stderr, "\n\t==> Error reading chunk of sinograms!\n\n");
          return rval;
        }
      } else {
        if (param.sino32BlockUse) {
          if ((rval = ReadBlockChunkOfSinograms(start_value, end_value, param,
                                                &h_sinoData[0])) != 0) {
            fprintf(stderr,
          ...
          if ((rval = ReadChunkOfSinograms(start_value, end_value, param,
                                           &h_sinoData[0])) != 0) {
            fprintf(stderr, "\n\t==> Error reading chunk of sinograms!\n\n");
            return rval;
          }
      ...
      const cudaExtent volumeSize =
          make_cudaExtent(param.imgWidth, param.imgHeight, nProjUse);
      //-ju- For 3D texture use: HANDLE_ERROR(cudaMalloc3DArray(&d_sino, &channelDesc, volumeSize));
      checkCudaErrors(cudaMalloc3DArray(&d_sino, &channelDesc, volumeSize,
                                        cudaArrayLayered));
      ...
      copyParams.srcPtr =
          make_cudaPitchedPtr(h_sinoData, volumeSize.width * sizeof(float),
      ...
      checkCudaErrors(
          cudaCreateTextureObject(&tex3DLayObj, &texRes, &texDescr, NULL));
      //02-02-2022////////////////////////////////////////////////////////////
      ...
      for (int iy = yChunkStart; iy < yChunkEnd; iy = iy + MAP * STREAM_SIZE) {
        ...
        for (int istm = 0; istm < STREAM_SIZE; istm++) {
          ...
          offset[istm] = (unsigned long long)param.volX *
                         (unsigned long long)param.volZ *
                         (unsigned long long)(index + istm * MAP);
          cudaMemcpyAsync(d_backProj[istm],
                          (char*)&h_backproj_chunk[offset[istm]],
                          MAP * param.volX * param.volZ * sizeof(float),
                          cudaMemcpyHostToDevice, stream[istm]);
          ...
          if (param.applyTiltCorr == 1) {
            ...
            if ((y_c % 100) == 0) {
              std::cout << "Using horizontal offset=" << offset_current
                        << " at slice=" << y_c << std::endl;
            }
          }
          ...
          if (param.useVolWin) {
            if (hasRemainder && np == proj_blcks - 1)  // process remainder
            {
              ...
              ///////////////////////////////////////////////////////////////////////
              if (param.highAcc == 0) {
                fdk_kernel_3DW_R<<<blocks, threads>>>(
                    d_backProj[istm], y_c, cuProjBlockIdx, remainProj,
                    param.volX, param.volZ, offset_current, param.shiftY,
                    param.wVolOrigX, param.wVolOrigY, param.wVolOrigZ,
                    tex3DLayObj /*02-02-2022 texture object*/);
              } else {
                fdk_kernel_3DW_R_HA<<<blocks, threads>>>(
                    d_backProj[istm], y_c, cuProjBlockIdx, remainProj,
                    param.volX, param.volZ, offset_current, param.shiftY,
                    param.wVolOrigX, param.wVolOrigY, param.wVolOrigZ,
                    tex3DLayObj /*02-02-2022 texture object*/);
              ...
              if (param.highAcc == 0) {
                ...
                fdk_kernel_3DW<<<blocks, threads>>>(
                    d_backProj[istm], y_c, cuProjBlockIdx, param.volX,
                    param.volZ, offset_current, param.shiftY, param.wVolOrigX,
                    param.wVolOrigY, param.wVolOrigZ,
                    tex3DLayObj /*02-02-2022 texture object*/);
              } else {
                fdk_kernel_3DW_HA<<<blocks, threads>>>(
                    d_backProj[istm], y_c, cuProjBlockIdx, param.volX,
                    param.volZ, offset_current, param.shiftY, param.wVolOrigX,
                    param.wVolOrigY, param.wVolOrigZ,
                    tex3DLayObj /*02-02-2022 texture object*/);
          ...
          cudaMemcpyAsync((char*)&h_backproj_chunk[offset[istm]],
                          d_backProj[istm],
                          MAP * param.volX * param.volZ * sizeof(float),
                          cudaMemcpyDeviceToHost, stream[istm]);
    ...
    for (int i = yChunkStart; i < yChunkEnd; i++) {
      ...
      unsigned long long offset = (unsigned long long)param.volX *
                                  (unsigned long long)param.volZ *
                                  (unsigned long long)index;
      //-ju-19-Oct-2011 calc. min/max
      unsigned long long endSeq = offset + (unsigned long long)param.volX *
                                               (unsigned long long)param.volZ;
      ...
      long long ii = i;
      // file offset
      unsigned long long fileOffset = ((unsigned long long)param.volY -
                                       (unsigned long long)(ii + startIdx)) *
                                      (unsigned long long)param.volX *
                                      (unsigned long long)param.volZ;
      if (param.tomoBlockWrite) {
        _fseeki64(fpVol, fileOffset * sizeof(float), 0);
        fwrite((char*)&h_backproj_chunk[offset],
               param.volX * param.volZ * sizeof(float), 1, fpVol);
        //index++;
      } else {
        //-ju-14-Apr-2015 --> add
        int id;
        ...
        sprintf(tomgramFile, "%s%s%04dx%04dx%04d_%04d.raw",
                param.tomoDir.c_str(), param.tomoName.c_str(), param.volX,
                param.volZ, param.volY, id);
        ...
        if (!s_backProjFile.write((char*)&h_backproj_chunk[offset],
