...
void PrintVersion(char* executable[]) {
  ...
  cout << "--------------------------------------------------------------------"
          "------------------------------------------\n-\n";
  if (!dwSize) {
    cout << "-  No Version Information for >> " << executable[0]
         << " << available\n";
    cout << "-\n---------------------------------------------------------------"
            "-----------------------------------------------\n";
    cout.flush();
  ...
  UINT len;
  r = VerQueryValueA(pVersionInfo, "\\VarFileInfo\\Translation",
                     (LPVOID*)&translation, &len);

  // comments
  char stringInfo[256];
  sprintf_s(stringInfo, 255, "\\StringFileInfo\\%04x%04x\\Comments",
  ...
  // copyright
  sprintf_s(stringInfo, 255, "\\StringFileInfo\\%04x%04x\\LegalCopyright",
            translation->language, translation->codePage);
  char* copyright;
