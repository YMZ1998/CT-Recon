...
////////////////////////////////////////////////////////
__forceinline__ __device__ void GetInterpolationPoint(
    const VolType& voxel, Point& ptInterpol, const float& phi, float& weight,
    float& winOrigX, float& winOrigY, float& winOrigZ) {
...
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
__global__ void fdk_kernel_3DW(
    float* d_backProj,   // accumulated back projected slice
    int y_c,             // current y-slice
    int cuProjBlockIdx,  // current projection block index to use
    int volWidth, int volDepth, float xOffset, float yOffset, float winOrigX,
    float winOrigY, float winOrigZ,
    cudaTextureObject_t tex3DLayObj)  //-ju-0202-2022 texture object
{
  ...
  float discr = sqrtf((x - volWidth_loc / 2.0f) * (x - volWidth_loc / 2.0f) +
                      (z - volWidth_loc / 2.0f) * (z - volWidth_loc / 2.0f));
  if (x < volWidth_loc && z < volDepth_loc && discr < volWidth_loc / 2.0f) {
    ...
    for (int i = 0; i < fdkConst.projProcSize; i++) {
      ...
      //-ju-08-Dec-2015
      for (int k = 0; k < MAP; k++) {
        GetInterpolationPoint(vox, interpolPoint, phi, l_weight, winOrigX,
                              winOrigY, winOrigZ);
        vst[k] +=
            tex2DLayered<float>(tex3DLayObj, interpolPoint.x + xOffset_loc,
                                interpolPoint.y + yOffset_loc, i) *
            l_weight;
        vox.y = vox.y + 1.0f;
      }
      vox.y = (float)y_c + 0.5f;
...
// kernel 1.2
__global__ void fdk_kernel_3DW_R(
    float* d_backProj,   // accumulated back projected slice
    int y_c,             // current y-slice
    int cuProjBlockIdx,  // current projection block index to use
    int cuProjBlocksize, int volWidth, int volDepth, float xOffset,
    float yOffset, float winOrigX, float winOrigY, float winOrigZ,
    cudaTextureObject_t tex3DLayObj)  //-ju-0202-2022 texture object
{
  //-ju-09-Dec-2015
  float vst[MAP];
...
///////////////////////////////////////////////////////////////////////////////////////////////
__global__ void fdk_kernel_3DW_HA(
    float* d_backProj,
    int y_c,             // current y-slice
    int cuProjBlockIdx,  // current projection block index to use
    int volWidth, int volDepth, float xOffset, float yOffset, float winOrigX,
    ...
    for (int i = 0; i < fdkConst.projProcSize; i++) {
      ...
      for (int k = 0; k < MAP; k++) {
        GetInterpolationPoint(vox, interpolPoint, phi, l_weight, winOrigX,
                              winOrigY, winOrigZ);
        vst[k] +=
            tex2DLayeredHighPrec(tex3DLayObj, interpolPoint.x + xOffset_loc,
                                 interpolPoint.y + yOffset_loc, i) *
            l_weight;
        vox.y = vox.y + 1.0f;
      }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void fdk_kernel_3DW_R_HA(
    float* d_backProj,   // accumulated back projected slice
    int y_c,             // current y-slice
    int cuProjBlockIdx,  // current projection block index to use
    int cuProjBlocksize, int volWidth, int volDepth, float xOffset,
