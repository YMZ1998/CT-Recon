...
// converter functions
//////////////////////
void ConvertTomoDataI16(const BackProjParam& param, const float& tomoMax,
                        const float& tomoMin) {
  ...
  for (int i = 0; i < param.volY; i++) {
    ...
    sprintf(tomgramFile, "%s%s%04dx%04dx%04d_%04d.raw", param.tomoDir.c_str(),
            param.tomoName.c_str(), param.volX, param.volY, param.volZ,
            i + param.startIndex);
    ...
    transform(readDataVector.begin(), readDataVector.end(), mapShort.begin(),
              RangeMapI16(mapRange, tomoMin));
...
void ConvertTomoDataUI16(const BackProjParam& param, const float& tomoMax,
                         const float& tomoMin) {
  ...
  for (int i = 0; i < param.volY; i++) {
    ...
    transform(readDataVector.begin(), readDataVector.end(), mapUShort.begin(),
              RangeMapUI16(mapRange, tomoMin));
...
////////////////////////////////////////
void ConvertTomoDataUI16MT(const BackProjParam& param, const float& tomoMax,
                           const float& tomoMin) {
  float mapRange = USHRT_MAX / (tomoMax - tomoMin);
  int tomoSize = param.volX * param.volZ;
  int onePercent = param.volY / 100;
...
void ConvertTomoDataUC8(const BackProjParam& param, const float& tomoMax,
                        const float& tomoMin) {
  ...
  for (int i = 0; i < param.volY; i++) {
    ...
    transform(readDataVector.begin(), readDataVector.end(), mapByte.begin(),
              RangeMapUC8(mapRange, tomoMin));
...
void ConvertBlockTomoDataUC8(const BackProjParam& param, const float& tomoMax,
                             const float& tomoMin) {
  ...
  for (int i = 0; i < param.volY; i++) {
    ...
    unsigned long long fileOffset =
        (unsigned long long)(i) * (unsigned long long)sliceSize;
...
void ConvertBlockTomoDataI16(const BackProjParam& param, const float& tomoMax,
                             const float& tomoMin) {
  FILE* fpVol = fopen(param.tomoBlockFile.c_str(), "r+b");
  if (fpVol == 0) {
    fprintf(stderr, "Can not open %s --> exit\n", param.tomoBlockFile.c_str());
...
void ConvertBlockTomoDataUI16(const BackProjParam& param, const float& tomoMax,
                              const float& tomoMin) {
  FILE* fpVol = fopen(param.tomoBlockFile.c_str(), "r+b");
  if (fpVol == 0) {
    fprintf(stderr, "Can not open %s --> exit\n", param.tomoBlockFile.c_str());
